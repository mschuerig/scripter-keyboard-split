...
your MIDI keyboard into up to four regions and sends each to its own
...
the way of two-handed playing. Add a third or fourth region when you
...
over into the other region. You can mix hard and smart splits in the
same setup.
...
the region that should trigger it.
...
With **Floating Range Above and Below set to 3**, each region's claim
extends `3` semitones past the split — so both regions can claim the
...
The same logic extends to more splits: each region tracks its own
...
**Nothing comes out, or only some regions sound.**
Check the MIDI channel of your receiving instrument. The Scripter
parameters show the channel each region is sent on; the instrument
...
**Notes occasionally jump to the wrong region.**
...
- Each region has a **claim zone** in pitch:
  ...
  A region can claim a pitch only if the pitch lies within that claim
  zone — the floating range is a hard upper bound on how far the
  region reaches past a split.
- For each incoming pitch:
  1. Gather every region whose claim zone contains the pitch.
  2. **Follow the hand:** if the *previous* note's region is among
     ...
     through the active region from being yanked across by a stale
     ...
     two-handed organ pattern, where the other region's recent note
     is far closer than the freshly-played one).
  3. Otherwise the region whose last-played pitch is closest in
     semitones wins; regions with no last pitch are treated as
     infinitely far away; ties go to the higher region index.
- **Hard split** is the same algorithm with both ranges of a split
  set to 0: the two adjacent regions' claim zones meet at the split
  point with no overlap (except the split point itself, which the
  tie-break sends to the upper region). Hard and smart splits can be
...
| File | Purpose |
|---|---|
| `split-router.js` | Single source of truth for the N-region routing algorithm. Pure JS, no Scripter dependencies — fully unit-testable. |
